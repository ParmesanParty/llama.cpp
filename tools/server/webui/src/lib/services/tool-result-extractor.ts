 ...
 * Non-paren punctuation (.,;:!?) is always stripped.
 * A trailing ')' is only stripped when it is unbalanced (more ')' than '(' in the string).
 * After each unbalanced ')' is dropped, non-paren punctuation is re-checked so that
 * sequences like "…page)." are fully cleaned.
 */
function stripTrailingPunctuation(url: string): string {
	// Strip non-paren trailing punctuation first.
	let stripped = url.replace(/[.,;:!?]+$/, '');
	// Then strip trailing ')' only when unbalanced.
	while (stripped.endsWith(')')) {
		const opens = (stripped.match(/\(/g) ?? []).length;
		const closes = (stripped.match(/\)/g) ?? []).length;
		if (closes > opens) {
			stripped = stripped.slice(0, -1);
			// A punctuation char may now sit at the tail — re-clean.
...
export function extractToolResult(raw: MCPRawToolCallResult): ExtractedToolResult {
	...
	if (raw._meta?.sources && Array.isArray(raw._meta.sources)) {
		...
		sources = raw._meta.sources
			...
			.map((s) => ({
				url: s.url,
				title: typeof s.title === 'string' && s.title.length > 0 ? s.title : s.url,
				published:
					typeof s.published === 'string' && s.published.length > 0 ? s.published : null,
