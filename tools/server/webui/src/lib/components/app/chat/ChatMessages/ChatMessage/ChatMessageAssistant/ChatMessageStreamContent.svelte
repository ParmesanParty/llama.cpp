<script lang="ts">
	...
	let retractedContent = $derived(
		retractionOffset !== null && retractionOffset > 0
			? content.slice(0, retractionOffset).trim()
			: null
	);
	...
	let effectiveContent = $derived(
		retractionOffset !== null ? content.slice(retractionOffset) : content
	);

	// Initial reasoning shown independently only when no retraction
	let initialReasoning = $derived(retractionOffset === null ? reasoning.initial : null);
	...
	let hasToolCalls = $derived(toolSteps.reduce((sum, step) => sum + step.tools.length, 0) > 0);
...
<div class="stream-content">
	<RetractionBlock content={retractedContent} reasoning={retractedReasoning} {attachments} />
	...
	{#if hasContentSegments}
		<MarkdownContent content={effectiveContent} {attachments} {sources} />
	{/if}
